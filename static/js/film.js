...
const FilmLab = (() => {
    ...
    function _bindFileInput() {
        ...
        if (input) {
            input.addEventListener("change", () => {
                ...
                _updateFileLabel(label, selectedFile);
    ...
    function _updateFileLabel(labelEl, file) {
        if (!labelEl) return;
        labelEl.textContent = file ? file.name : "No file selected";
    }
    ...
    function processPhoto() {
        ...
        if (statusEl) { statusEl.textContent = ""; statusEl.classList.remove("error-msg"); }
        ...
        fetch("/api/film/process", { method: "POST", body: form })
            .then(r => {
                if (!r.ok) return r.json().then(d => Promise.reject(d.error || "Processing failed."));
                return r.blob();
            })
            .then(blob => {
                ...
                a.href = url;
                a.download = `${base}_film.jpg`;
                a.click();
                URL.revokeObjectURL(url);
                if (statusEl) statusEl.textContent = "Done — file downloaded.";
            })
            .catch(err => {
                if (statusEl) {
            ...
            .finally(() => {
                processing = false;
                if (btn) { btn.disabled = false; btn.textContent = "Process & Download"; }
            });
    }
